...
const payloadSchemaBody = (
	schema: Schema.Schema.All,
): Schema.Schema<any, HttpBody.HttpBody> => {
	...
	const transform = Schema.transformOrFail(HttpBodyFromSelf, schema, {
		decode(fromA, _, ast) {
			return ParseResult.fail(
				new ParseResult.Forbidden(ast, fromA, "encode only schema"),
			);
		},
		encode(toI, _, ast) {
			switch (encoding.kind) {
						...
						ParseResult.mapError(
							(error) =>
								new ParseResult.Type(
									ast,
									toI,
									`Could not encode as JSON: ${error}`,
								),
						),
					);
				}
				case "Text": {
					if (typeof toI !== "string") {
						return ParseResult.fail(
							new ParseResult.Type(ast, toI, "Expected a string"),
						);
					}
				...
				case "UrlParams": {
					return ParseResult.succeed(
						HttpBody.urlParams(UrlParams.fromInput(toI as any)),
					);
				}
				case "Uint8Array": {
					if (!(toI instanceof Uint8Array)) {
						return ParseResult.fail(
							new ParseResult.Type(ast, toI, "Expected a Uint8Array"),
						);
					}
...
> = Simplify<
	{
		...
		readonly [Method in UnsafeClient.TopLevelMethods<
			Groups,
			ApiError
...
export declare namespace UnsafeClient {
	...
	>
		? _Endpoints extends infer Endpoint
			? [
					HttpApiEndpoint.HttpApiEndpoint.Name<Endpoint>,
					Method<Endpoint, ApiError, _Error>,
				]
			: never
		: never;
}
...
>(
	...
	options: {
		...
		readonly onEndpoint: (options: {
			readonly group: HttpApiGroup.HttpApiGroup.AnyWithProps;
			readonly endpoint: HttpApiEndpoint.HttpApiEndpoint<
				string,
					...
					Effect.gen(function* () {
						...
						if (request?.payload instanceof FormData) {
							...
							if (HttpMethod.hasBody(endpoint.method)) {
								...
								const urlParams = (yield* encodePayloadBody.value(
									request?.payload,
								)) as Record<string, string>;
								httpRequest = HttpClientRequest.setUrlParams(
									httpRequest,
									urlParams,
								);
							}
...
> => {
	...
	return makeUnsafeClientInternal(api, {
		...
		onEndpoint({ endpoint, endpointFn, group }) {
			(group.topLevel ? client : (client[group.identifier] as any))[
				endpoint.name
			] = endpointFn;
		},
